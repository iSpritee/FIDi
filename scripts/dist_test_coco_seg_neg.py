...
def validate(args=None):
    ...
    checkpoint_path = 'work_dir_coco/dino+affinity0.1/checkpoints/model_iter_132000.pth'
    ...
    pred_dir = os.path.join(base_dir, "Pred1", args.val_set)  
    ...
    with torch.no_grad():
        ...
        for _, data in tqdm(enumerate(val_loader), total=len(val_loader), ncols=100, ascii=" >=", 
            ...
            if args.use_crf:
                ...
                # 根据 split 选择原图目录
                if args.val_set in ['val', 'val_part', 'test', 'val2014']:
                    img_subdir = 'val2014'
                else:
                    img_subdir = 'train2014'

                img_path = os.path.join(args.img_folder, img_subdir, img_name + '.jpg')
                img_pil = Image.open(img_path).convert('RGB')
                img_pil = img_pil.resize((W, H), Image.BILINEAR)
                img_temp = np.array(img_pil).astype(np.uint8)   # [H, W, 3]

                # 只保留当前图像可能出现的类别：背景 + cls_label中的正类
                cls_np = cls_label[0].detach().cpu().numpy()   # [C]
                valid_cls = np.where(cls_np > 0)[0].tolist()
                if 0 not in valid_cls:
                    valid_cls = [0] + valid_cls

                valid_probs = probs[valid_cls, :, :]
                valid_probs = valid_probs / (np.sum(valid_probs, axis=0, keepdims=True) + 1e-8)
                ...
                crf_probs = crf_inference(
                    ...
                    t=5,
                    scale_factor=1,
                    labels=len(valid_cls)
                )
                pred_idx = np.argmax(crf_probs, axis=0)
                pred_mask = np.array(valid_cls, dtype=np.uint8)[pred_idx]
            else:
            ...
            probs = torch.softmax(avg_logits, dim=1).squeeze(0).cpu().numpy()  # [C, H, W]
            ...
            #     img_path = os.path.join(args.img_folder, 'val2014', img_name + '.jpg')
            #     img_pil = Image.open(img_path).convert('RGB')
    ...
    tab_results = format_tabs([seg_score], name_list=["Pred"], cat_list=coco.class_list)
...
TRANSFORMERS_OFFLINE=1 CUDA_VISIBLE_DEVICES=0 torchrun \
    --standalone \
    --nproc_per_node=1 \
    scripts/dist_test_coco_seg_neg.py \
