...
from transformers import CLIPTextModel, CLIPTokenizer, ViTModel
...
class ViTMaskRefiner(nn.Module):
    ...
    def forward(self, pseudo_logits, vit_feat):
        """
        Args:
            coarse_mask: [B, K, H, W]  SD生成的伪标签(Logits或Prob) [1,21,500,333]
            vit_feat:   [B, C, h, w]  ViT提取的特征图 #[1,768,14,14]
        """
        h, w = vit_feat.shape[-2:]
        ...
        affinity = self.get_affinity(vit_feat)
...
class DiffusionBasedNetwork(nn.Module):
    def __init__(
        ...
        self.vit_embed_dim = self.feature_extractor.config.hidden_size
        ...
        self.vit_refiner = ViTMaskRefiner(in_channels=self.vit_embed_dim, beta=10, iter_num=1, alpha=0.1)
        ...
        self.vit_projector = nn.Sequential(
            nn.Conv2d(self.vit_embed_dim * self.n_last_blocks, self.vit_embed_dim, kernel_size=1, bias=False),
            nn.GroupNorm(16, self.vit_embed_dim),
            nn.ReLU(inplace=True)
        )

        self.seg_head = nn.Sequential(
            nn.Conv2d(self.vit_embed_dim, 256, 3, padding=1, bias=False),
            nn.GroupNorm(16, 256),
            nn.ReLU(inplace=True),
            nn.Dropout2d(0.1),
    ...
    def extract_feature(self, x):
        ...
        patch_size = self.feature_extractor.config.patch_size
        h_grid = H // patch_size
        w_grid = W // patch_size
        num_patches = h_grid * w_grid

        # 提取最后四个 Transformer Block 的 Patch Tokens
        last_n_states = outputs.hidden_states[-self.n_last_blocks:] 

        cat_feat = []
        for state in last_n_states:
            # 第一个 token 是 CLS token，只保留具有空间位置的 Patch Tokens
            patch_tokens = state[:, 1:, :]
            assert patch_tokens.shape[1] == num_patches, (
                f"Patch token 数量错误：得到 {patch_tokens.shape[1]}，预期 {num_patches}"
            )
            feat = patch_tokens.permute(0, 2, 1).reshape(B, self.vit_embed_dim, h_grid, w_grid)
            cat_feat.append(feat)

        fused_feat = torch.cat(cat_feat, dim=1)

        vit_feat = self.vit_projector(fused_feat)
    ...
    def forward(self, x, cls_label, mask=None, img_name=None):
        ...
        image_size = self.feature_extractor.config.image_size
        x_vit = F.interpolate(x, size=(image_size, image_size), mode='bilinear', align_corners=False)
        vit_feat = self.extract_feature(normalizer(x_vit)) 

        # 用 ViT 优化伪预测得到优化后的伪标签
        refined_pseudo_logits = self.vit_refiner(pseudo_logits_low.detach(), vit_feat) # [1,21,14,14]
    ...
    def get_param_groups(self):
        ...
        for param in self.vit_projector.parameters():
            param_group[2].append(param)
            
        for param in self.vit_refiner.parameters():
            param_group[2].append(param)
