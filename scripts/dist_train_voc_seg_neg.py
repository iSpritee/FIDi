...
parser.add_argument("--max_iters", default=50000, type=int, help="max training iters")
...
parser.add_argument("--warmup_iters", default=8000, type=int, help="warmup_iters")
...
def validate(model=None, data_loader=None, args=None):
    ...
    with torch.no_grad():
        ...
        for _, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >=", 
            ...
            _, _, pred, refined_label = model(inputs, cls_label, labels)
            ...
            Masks.append(refined_label[0].cpu().numpy().astype(np.int16))
            GTs.append(labels[0].cpu().numpy().astype(np.int16))
            
    pred_score = evaluate.scores(GTs, Preds)
    mask_score = evaluate.scores(GTs, Masks)
    model.train()

    tab_results = format_tabs([pred_score, mask_score], name_list=["Pred", "Mask"], cat_list=voc.class_list)
    # tab_results = format_tabs([mask_score], name_list=["Mask"], cat_list=voc.class_list)
...
def train(args=None):
    ...
    for n_iter in pbar:
        ...
        label_loss = get_seg_loss(pseudo_logits, attn_label.type(torch.long), ignore_index=args.ignore_index)
        ...
        if (n_iter + 1) >=40000 and (n_iter + 1) % args.eval_iters == 0:
